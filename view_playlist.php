    ...
    <div class="col-md-8">
      ...
      <table class="table bg-black">
        ...
        <tbody>
          <?php
          $i = 0;
          $items = $conn->query("SELECT p.*,m.title,m.artist,m.upath FROM playlist_items p inner join uploads m on m.id = p.music_id where p.playlist_id =$id ");
          while ($row = $items->fetch_assoc()):
            ?>
            <tr class="pitem" data-queue="<?php echo $i ?>" data-id="<?php echo $row['music_id'] ?>"
              ...
              <td>
                <button class="btn btn-sm btn-danger"
                  onclick="deleteMusic(<?php echo $row['music_id'] ?>)">Delete</button>
              </td>
            </tr>
            <?php
            $i++;
...
<script>
  // Function to delete a music item from the playlist
  function deleteMusic(musicId) {
    // Confirmation prompt before deleting the music item
    if (confirm("Are you sure you want to delete this music item?")) {
      // AJAX request to delete_music.php
      $.ajax({
        ...
        error: function (xhr, status, error) {
          ...
          alert('An error occurred while deleting the music item.');
        }
      });
    }
  }
