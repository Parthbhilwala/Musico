...
<div class="col-lg-6 m-auto">
  <table class="text-white text-center" border='1'>
    <thead>
      <tr>
        <th>No.</th>
    ...
    <tbody>
      <?php
      $payers = "SELECT p.*, CONCAT(firstname, ' ', lastname) AS full_name FROM pay p 
    LEFT JOIN users u ON p.user_id = u.id";
      $result = $conn->query($payers);
      ...
      while ($row = $result->fetch_assoc()) {
        ...
        <tr>
          ...
          <td>
            <?php
            $newDate = date("F j, Y", strtotime($row['added_on']));
            echo $newDate; ?>
          </td>
          <td>
